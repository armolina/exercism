...
def is_criticality_balanced(temperature, neutrons_emitted):
    ...
    max_temperature = 800
    max_neutron_emision = 500
    max_temperature_neutrons_product = 500000

    if temperature < max_temperature \
        and neutrons_emitted > max_neutron_emision \
            and (temperature*neutrons_emitted) < max_temperature_neutrons_product:
        return True
    else:
        return False
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    temperatue_netutrons_threshold_percentage = ((temperature * neutrons_produced_per_second) * 100) / threshold

    if temperatue_netutrons_threshold_percentage < 90:
        return "LOW"
